...
// OK
    hotbitApi.prototype.getMarketOrders = function (symbol)
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            var symbolreply = symbol.replace("_", "");

            symbol = symbol.replace("_", "/");

            var qs1 = 'api_key=' + this.api_key + '&limit=100&market=' + symbol + '&offset=0';
            ...
            request.post(this.apiURL + '/order.pending', {json:true, form: postbody}, function (error, response, body)
            {
                ...
                resolve(body.result[symbolreply].records);
...
// OK
    hotbitApi.prototype.createOrder = function (symbol, side, amount, price)
    {
        ...
        var qs1 = 'amount=' + Big(amount).toFixed(8) + '&api_key=' + this.api_key + '&isfee=0&market=' + symbol.replace("_", "/") + '&price=' + Big(price).toFixed(10) + '&side=' + (side.toLowerCase() == 'sell'?1:2);
    ...
    hotbitApi.prototype.cancelAllMarketOrders = function (symbol)
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.post(this.apiURL + '/order.pending', {json:true, form: postbody}, function (error, response, body)
            {
                ...
                for (i = 0; i < body.result[symbolreply].records.length; i++)
                {

                    var orderdata = body.result[symbolreply].records[i];

                    var qs1 = 'api_key=' + this.api_key + '&market=' + symbol + '&order_id' + orderdata.id;

                    var qs2 = qs1 + '&secret_key=' + this.sign;
            
                    var hash = crypto.createHash('md5').update(qs2).digest('hex').toUpperCase();
            
                    var cpostbody = qs1 + '&sign=' + hash;
    ...
    hotbitApi.prototype.getOrderById = function (symbol, clientOrderId)
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.post(this.apiURL + '/order.pending', {json:true, form: postbody}, function (error, response, body)
            {
                ...
                var qs1 = 'api_key=' + this.api_key + '&end_time=1999999999&limit=100&market=' + symbol + '&offset=0&side=1&start_time=0';

                var qs2 = qs1 + '&secret_key=' + this.sign;
        
                var hash = crypto.createHash('md5').update(qs2).digest('hex').toUpperCase();
        
                var postbody2 = qs1 + '&sign=' + hash;
                ...
                request.post(this.apiURL + '/order.finished', {json:true, form: postbody}, function (error, response, body2)
                {
                    ...
                    var qs1 = 'api_key=' + this.api_key + '&end_time=1999999999&limit=100&market=' + symbol + '&offset=0&side=2&start_time=0';

                    var qs2 = qs1 + '&secret_key=' + this.sign;
            
                    var hash = crypto.createHash('md5').update(qs2).digest('hex').toUpperCase();
            
                    var postbody3 = qs1 + '&sign=' + hash;
...
// OK
    hotbitApi.prototype.cancelOrderById = function (symbol, clientOrderId)
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            var qs1 = 'api_key=' + this.api_key + '&market=' + symbol + '&order_id=' + clientOrderId;
...
// OK
    hotbitApi.prototype.getTradesByOrderId = function (orderId)
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            var qs1 = 'api_key=' + this.api_key + '&limit=100&market=' + symbol + '&offset=0&order_id=' + orderId;
