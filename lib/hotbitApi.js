...
// OK
    hotbitApi.prototype.getAccountBalances = function (assets)
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            parameters['asssets'] = JSON.stringify(assets);

            var postbody = querystring.stringify(parameters); 

            var hmac = crypto.createHmac('sha256', this.sign);
            hmac.update(postbody);
            var signature = hmac.digest('hex');

            postbody += '&sign=' + signature;
